...
ItemListFragment.Callbacks {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (findViewById(R.id.item_detail_container) != null) {
			...
			if(fragment == null){
				...
				trans.replace(R.id.item_list, new ItemListFragment());
				trans.commit();
	...
	public boolean onOptionsItemSelected(MenuItem item){
		...
		case android.R.id.home:
			//we replace with a whole new fragment
			//can be improved with a save from the previous one and restore
			FragmentTransaction trans = this.getSupportFragmentManager().beginTransaction();
			trans.replace(R.id.item_list, new ItemListFragment());
			trans.commit();
	...
	public void onItemSelected(String id) {
		if (mTwoPane) {
			...
			//lol? seriously google?
			//Intent detailIntent = new Intent(this, ItemDetailActivity.class);
			//detailIntent.putExtra(ItemDetailFragment.ARG_ITEM_ID, id);
			//startActivity(detailIntent);

			//we want to manage the top return
			this.getActionBar().setDisplayHomeAsUpEnabled(true);
			...
			FragmentTransaction trans = this.getSupportFragmentManager().beginTransaction();
			trans.replace(R.id.item_list, fragment);
			trans.commit();
		}
	}
}
